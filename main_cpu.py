"""
Main script to run full SARRP CBCT reconstruction on CPU using pure backproject functions.

Loads real projection data and reconstructs a 3D volume using CPU-only backprojection
from backproject.py, projector.py, and geometry.py.
"""
...
def main():
    """Main CPU-only reconstruction pipeline."""
    
    parser = argparse.ArgumentParser(
        description="SARRP CBCT CPU-only reconstruction using backproject module"
    )
    parser.add_argument(
        "--config",
        type=str,
